...
public class RedisInputStream extends FilterInputStream {
  ...
  public byte readByte() throws UodisConnectionException {
    ensureFill();
    return buf[count++];
  }
  ...
  public String readLine() {
    ...
    if (reply.length() == 0) {
      throw new UodisConnectionException("It seems like server has closed the connection.");
    }
  ...
  public long readLongCrLf() {
    ...
    while (true) {
      ...
      if (b == '\r') {
        ...
        if (buf[count++] != '\n') {
          throw new UodisConnectionException("Unexpected character!");
        }
  ...
  @Override
  public int read(byte[] b, int off, int len) throws UodisConnectionException {
    ensureFill();
   ...
   */
  private void ensureFill() throws UodisConnectionException {
    if (count >= limit) {
      try {
        limit = in.read(buf);
        count = 0;
        if (limit == -1) {
          throw new UodisConnectionException("Unexpected end of stream.");
        }
      } catch (IOException e) {
        throw new UodisConnectionException(e);
      }
    }
  }
}
