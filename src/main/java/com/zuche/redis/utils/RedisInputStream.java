...
public class RedisInputStream extends FilterInputStream {
  ...
  public byte readByte() throws FrameworkRedisConnectionException {
    ensureFill();
    return buf[count++];
  }
  ...
  public String readLine() {
    ...
    if (reply.length() == 0) {
      throw new FrameworkRedisConnectionException("It seems like server has closed the connection.");
    }
  ...
  public long readLongCrLf() {
    ...
    while (true) {
      ...
      if (b == '\r') {
        ...
        if (buf[count++] != '\n') {
          throw new FrameworkRedisConnectionException("Unexpected character!");
        }
  ...
  @Override
  public int read(byte[] b, int off, int len) throws FrameworkRedisConnectionException {
    ensureFill();
   ...
   */
  private void ensureFill() throws FrameworkRedisConnectionException {
    if (count >= limit) {
      try {
        limit = in.read(buf);
        count = 0;
        if (limit == -1) {
          throw new FrameworkRedisConnectionException("Unexpected end of stream.");
        }
      } catch (IOException e) {
