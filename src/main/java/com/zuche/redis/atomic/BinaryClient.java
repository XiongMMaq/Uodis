package com.zuche.redis.atomic;


import com.zuche.redis.params.*;
import com.zuche.redis.support.GeoCoordinate;
import com.zuche.redis.support.GeoUnit;
import com.zuche.redis.support.ListPosition;
...
import static com.zuche.redis.atomic.Protocol.Command.*;
import static com.zuche.redis.atomic.Protocol.Command.EXISTS;
import static com.zuche.redis.atomic.Protocol.Command.GET;
import static com.zuche.redis.atomic.Protocol.Command.PING;
import static com.zuche.redis.atomic.Protocol.Command.PSUBSCRIBE;
import static com.zuche.redis.atomic.Protocol.Command.PUNSUBSCRIBE;
import static com.zuche.redis.atomic.Protocol.Command.SET;
import static com.zuche.redis.atomic.Protocol.Command.SUBSCRIBE;
import static com.zuche.redis.atomic.Protocol.Command.UNSUBSCRIBE;
import static com.zuche.redis.atomic.Protocol.Keyword.*;
...
public class BinaryClient extends Connection {
    ...
    public void zinterstore(final byte[] dstkey, final byte[]... sets) {
        sendCommand(ZINTERSTORE, joinParameters(dstkey, toByteArray(sets.length), sets));
    }
    ...
    public void configGet(final byte[] pattern) {
        sendCommand(CONFIG, Protocol.Keyword.GET.raw, pattern);
    }

    public void configSet(final byte[] parameter, final byte[] value) {
        sendCommand(CONFIG, Protocol.Keyword.SET.raw, parameter, value);
    }
    ...
    public void configResetStat() {
        sendCommand(CONFIG, Protocol.Keyword.RESETSTAT.raw);
    }

    public void configRewrite() {
        sendCommand(CONFIG, Protocol.Keyword.REWRITE.raw);
    }
    ...
    public void scriptFlush() {
        sendCommand(SCRIPT, Protocol.Keyword.FLUSH.raw);
    }

    public void scriptExists(final byte[]... sha1) {
        sendCommand(SCRIPT, joinParameters(Protocol.Keyword.EXISTS.raw, sha1));
    }

    public void scriptLoad(final byte[] script) {
        sendCommand(SCRIPT, Protocol.Keyword.LOAD.raw, script);
    }

    public void scriptKill() {
        sendCommand(SCRIPT, Protocol.Keyword.KILL.raw);
    }

    public void slowlogGet() {
        sendCommand(SLOWLOG, Protocol.Keyword.GET.raw);
    }

    public void slowlogGet(final long entries) {
        sendCommand(SLOWLOG, Protocol.Keyword.GET.raw, toByteArray(entries));
    }
    ...
    public void clientKill(final byte[] client) {
        sendCommand(CLIENT, Protocol.Keyword.KILL.raw, client);
    }

    public void clientGetname() {
        sendCommand(CLIENT, Protocol.Keyword.GETNAME.raw);
    }

    public void clientList() {
        sendCommand(CLIENT, Protocol.Keyword.LIST.raw);
    }

    public void clientSetname(final byte[] name) {
        sendCommand(CLIENT, Protocol.Keyword.SETNAME.raw, name);
    }
    ...
    public void moduleLoad(final byte[] path) {
        sendCommand(MODULE, Protocol.Keyword.LOAD.raw, path);
    }

    public void moduleList() {
        sendCommand(MODULE, Protocol.Keyword.LIST.raw);
    }

    public void moduleUnload(final byte[] name) {
        sendCommand(MODULE, Protocol.Keyword.UNLOAD.raw, name);
    }
