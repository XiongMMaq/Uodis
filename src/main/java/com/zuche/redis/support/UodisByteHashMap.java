 ...
 * @description:
 */
public class UodisByteHashMap implements Map<byte[],byte[]>,Cloneable,Serializable {
    ...
    public Set<Entry<byte[], byte[]>> entrySet() {
        ...
        while (iterator.hasNext()) {
            ...
            hashSet.add(new UodisByteEntry(entry.getKey().data, entry.getValue()));
        }
        return hashSet;
    }
    ...
    private static final class UodisByteEntry implements Entry<byte[], byte[]> {
        private byte[] value;
        private byte[] key;

        public UodisByteEntry(byte[] key, byte[] value) {
            this.key = key;
            this.value = value;
        }
