package com.zuche.redis;


import com.zuche.redis.atomic.BuilderFactory;
import com.zuche.redis.atomic.Client;
import com.zuche.redis.commands.*;
import com.zuche.redis.exceptions.FrameworkRedisDataException;
import com.zuche.redis.exceptions.FrameworkRedisException;
import com.zuche.redis.exceptions.InvalidURIException;
import com.zuche.redis.params.*;
import com.zuche.redis.support.*;
import com.zuche.redis.utils.UodisURIHelper;
...
import static com.zuche.redis.atomic.Protocol.toByteArray;
...
public class BinaryUodis implements BasicCommands, BinaryUodisCommands, MultiKeyBinaryCommands,
    ...
    public BinaryUodis(final String host) {
        URI uri = URI.create(host);
        if (UodisURIHelper.isValid(uri)) {
    ...
    public BinaryUodis(final HostAndPort hp) {
        this(hp.getHost(), hp.getPort());
    }

    public BinaryUodis(final String host, final int port) {
        client = new Client(host, port);
    }

    public BinaryUodis(final String host, final int port, final boolean ssl) {
    ...
    public BinaryUodis(final String host, final int port, final int timeout) {
        this(host, port, timeout, timeout);
    }

    public BinaryUodis(final String host, final int port, final int timeout, final boolean ssl) {
    ...
    public BinaryUodis(final String host, final int port, final int connectionTimeout,
                       final int soTimeout) {
        client = new Client(host, port);
    ...
    public BinaryUodis(URI uri) {
        initializeClientFromURI(uri);
    }

    public BinaryUodis(URI uri, final SSLSocketFactory sslSocketFactory,
                       final SSLParameters sslParameters, final HostnameVerifier hostnameVerifier) {
        initializeClientFromURI(uri, sslSocketFactory, sslParameters, hostnameVerifier);
    }

    public BinaryUodis(final URI uri, final int timeout) {
        this(uri, timeout, timeout);
    }

    public BinaryUodis(final URI uri, final int timeout, final SSLSocketFactory sslSocketFactory,
                       final SSLParameters sslParameters, final HostnameVerifier hostnameVerifier) {
        this(uri, timeout, timeout, sslSocketFactory, sslParameters, hostnameVerifier);
    }

    public BinaryUodis(final URI uri, final int connectionTimeout, final int soTimeout) {
        initializeClientFromURI(uri);
        client.setConnectionTimeout(connectionTimeout);
        client.setSoTimeout(soTimeout);
    }
    ...
    private void initializeClientFromURI(URI uri, final SSLSocketFactory sslSocketFactory,
        ...
        client = new Client(uri.getHost(), uri.getPort(), UodisURIHelper.isRedisSSLScheme(uri),
                sslSocketFactory, sslParameters, hostnameVerifier);

        String password = UodisURIHelper.getPassword(uri);
        if (password != null) {
            client.auth(password);
            client.getStatusCodeReply();
        }

        int dbIndex = UodisURIHelper.getDBIndex(uri);
        if (dbIndex > 0) {
    ...
    public Map<byte[], byte[]> hgetAll(final byte[] key) {
        ...
        final Map<byte[], byte[]> hash = new UodisByteHashMap();
     ...
     * operation.
     * @see BinaryUodis#rpush(byte[], byte[]...)
     */
    @Override
    ...
    protected void checkIsInMultiOrPipeline() {
        if (client.isInMulti()) {
            throw new FrameworkRedisDataException(
                    "Cannot use Uodis when in Multi. Please use Transaction or reset jedis state.");
        } else if (pipeline != null && pipeline.hasPipelinedResponse()) {
            throw new FrameworkRedisDataException(
                    "Cannot use Uodis when in Pipeline. Please use Pipeline or reset jedis state .");
        }
    }
    ...
    public String shutdown() {
        ...
        } catch (FrameworkRedisException ex) {
            status = null;
        }
        return status;
    }
     ...
     * @param uodisMonitor
     */
    public void monitor(final UodisMonitor uodisMonitor) {
        client.monitor();
        client.getStatusCodeReply();
        uodisMonitor.proceed(client);
    }
    ...
    @Override
    public void subscribe(BinaryUodisPubSub jedisPubSub, final byte[]... channels) {
        client.setTimeoutInfinite();
        try {
    ...
    @Override
    public void psubscribe(BinaryUodisPubSub jedisPubSub, final byte[]... patterns) {
        client.setTimeoutInfinite();
        try {
